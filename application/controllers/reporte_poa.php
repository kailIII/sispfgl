...
//2.5
    ...
    public function reportesPOATrimestral() {
        $anio = $this->input->post('anio');
        $numTrimestre = $this->input->post('trimestre');
        switch ($numTrimestre) {
            case 1:
                $trimestre = "Primer";
                break;
        ...
        $estEnc4 = array(
            'font' => array('bold' => true, 'size' => 10, 'name' => 'Arial', 'color' => array('rgb' => 'FFFFFF')),
            'fill' => array('type' => PHPExcel_Style_Fill::FILL_SOLID, 'color' => array('rgb' => 'FB6464')),
            'alignment' => array('horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
                'vertical' => PHPExcel_Style_Alignment::VERTICAL_JUSTIFY,
                'wrapText' => true,
                'shrinkToFit' => true
...
Préstamo BIRF 7916');
        ...
        $this->phpexcel->getActiveSheet()->setCellValue('Q15', "Observaciones");
        $this->phpexcel->getActiveSheet()->getColumnDimension('Q')->setWidth(25);
        $this->phpexcel->getActiveSheet()->getColumnDimension('R')->setWidth(25);
        $this->phpexcel->getActiveSheet()->getColumnDimension('S')->setWidth(25);
        $this->phpexcel->getActiveSheet()->getColumnDimension('T')->setWidth(25);
        $this->phpexcel->getActiveSheet()->mergeCells('Q15:Q16');
        $this->phpexcel->getActiveSheet()->getStyle('Q15:Q16')->applyFromArray($estEnc4);

        $this->phpexcel->getActiveSheet()->setCellValue('R15', "Seguimiento del $trimestre Trimestre");
        $this->phpexcel->getActiveSheet()->mergeCells('R15:T15');

        $this->phpexcel->getActiveSheet()->setCellValue('R16', "Nivel de cumplimiento trimestral acumulado (%)");
        $this->phpexcel->getActiveSheet()->setCellValue('S16', "Valoración del avance (Por qué, razones del avance obtenido)");
        $this->phpexcel->getActiveSheet()->setCellValue('T16', "Decisiones o medidas correctivas");
        $this->phpexcel->getActiveSheet()->getStyle('R15:T16')->applyFromArray($estEnc4);
        ...
        foreach ($actividades as $aux) {
            if (!is_null($aux->poa_act_padre)) {
                ...
                $detalleTrimestral = $this->trimestral->obtenerActividadTrimestral($anio, $aux->poa_act_id, $numTrimestre);
                $this->phpexcel->getActiveSheet()->setCellValue("Q$i", $detalleTrimestral->poa_act_seg_tri_observacion);
                $this->phpexcel->getActiveSheet()->setCellValue("R$i", $detalleTrimestral->poa_act_seg_tri_nivel);
                $this->phpexcel->getActiveSheet()->setCellValue("S$i", $detalleTrimestral->poa_act_seg_tri_valoracion);
                $this->phpexcel->getActiveSheet()->setCellValue("T$i", $detalleTrimestral->poa_act_seg_tri_decision);


                $this->phpexcel->getActiveSheet()->getStyle("B$i:T$i")->applyFromArray($estCuerpo);
            } else {
                $this->phpexcel->getActiveSheet()->setCellValue("B$i", $aux->poa_act_codigo);
                $this->phpexcel->getActiveSheet()->setCellValue("C$i", $aux->poa_act_descripcion);
                $this->phpexcel->getActiveSheet()->mergeCells("C$i:P$i");
                $this->phpexcel->getActiveSheet()->mergeCells("Q$i:T$i");
                $this->phpexcel->getActiveSheet()->getStyle("B$i:T$i")->applyFromArray($estCuerpoPadre);
            }
            $i++;
        }
        /* HOJA 2-5 */
...
Préstamo BIRF 7916');
                ...
                $this->phpexcel->getActiveSheet()->getRowDimension('15')->setRowHeight(25);
                $this->phpexcel->getActiveSheet()->getRowDimension('16')->setRowHeight(25);
                //SEGUIMIENTO TRIMESTRAL ENCABEZADO
                $this->phpexcel->getActiveSheet()->setCellValue('AT15', "Observaciones");
                $this->phpexcel->getActiveSheet()->mergeCells('AT15:AT16');
                $this->phpexcel->getActiveSheet()->setCellValue('AU15', "Seguimiento del $trimestre Trimestre");
                $this->phpexcel->getActiveSheet()->mergeCells('AU15:AW15');
                $this->phpexcel->getActiveSheet()->setCellValue('AU16', "Nivel de cumplimiento trimestral acumulado (%)");
                $this->phpexcel->getActiveSheet()->setCellValue('AV16', "Valoración del avance (Por qué, razones del avance obtenido)");
                $this->phpexcel->getActiveSheet()->setCellValue('AW16', "Decisiones o medidas correctivas");
                $this->phpexcel->getActiveSheet()->getStyle('AT15:AW16')->applyFromArray($estEnc4);
                $this->phpexcel->getActiveSheet()->getColumnDimension('AT')->setWidth(25);
                $this->phpexcel->getActiveSheet()->getColumnDimension('AU')->setWidth(25);
                $this->phpexcel->getActiveSheet()->getColumnDimension('AV')->setWidth(25);
                $this->phpexcel->getActiveSheet()->getColumnDimension('AW')->setWidth(25);
                ...
                foreach ($actividades as $aux) {
                    ...
                    $detalleTrimestral = $this->trimestral->obtenerActividadTrimestral($anio, $aux->poa_act_id, $numTrimestre);
                    $this->phpexcel->getActiveSheet()->setCellValue("AT$i", $detalleTrimestral->poa_act_seg_tri_observacion);
                    $this->phpexcel->getActiveSheet()->setCellValue("AU$i", $detalleTrimestral->poa_act_seg_tri_nivel);
                    $this->phpexcel->getActiveSheet()->setCellValue("AV$i", $detalleTrimestral->poa_act_seg_tri_valoracion);
                    $this->phpexcel->getActiveSheet()->setCellValue("AW$i", $detalleTrimestral->poa_act_seg_tri_decision);
                    $this->phpexcel->getActiveSheet()->getStyle("A$i:AW$i")->applyFromArray($estCuerpo);
                    $i++;
                }
            }
        }
...
Préstamo BIRF 7916');
         ...
         * SALIDA DEL DOCUMENTO
         */
        $filename = "POA_Trimestral_" . date("d-m-y") . ".xls"; //GUARDANDO CON ESTE NOMBRE
        header('Content-Type: application/vnd.ms-excel');
        header("Content-Disposition: attachment;filename=$filename");
        header('Cache-Control: max-age=0');
        $objWriter = PHPExcel_IOFactory::createWriter($this->phpexcel, 'Excel5');

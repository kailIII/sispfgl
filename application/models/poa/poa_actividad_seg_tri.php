...
class Poa_actividad_seg_tri extends CI_Model {
    ...
    public function obtenerActividadDetalleTri($anio, $poa_com_id) {
...
WHERE A.poa_act_id=B.poa_act_id AND B.poa_act_det_id=C.poa_act_det_id
	AND A.poa_com_id=? AND B.poa_act_det_anio = ?";
    ...
    public function obtenerActividadTri($anio, $poa_com_id) {
        $sql = "SELECT COUNT(C.poa_act_det_id) valor
FROM poa_actividad A, poa_actividad_detalle B, $this->tabla C
WHERE A.poa_act_id=B.poa_act_id AND B.poa_act_det_id=C.poa_act_det_id
	AND A.poa_act_id=? AND B.poa_act_det_anio = ?";
        $consulta = $this->db->query($sql, array($poa_com_id,$anio));
        $resultado = $consulta->result();
        return $resultado[0];
